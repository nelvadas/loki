...
type sampleStream struct {
	...
	// labels: that is what makes it equal to the fingerprint the TSDB index gives the store's chunks
	// and identical across ingesters — unaffected by the fp mapper's collision remapping (unlike
	// stream.fp) or by any label-reducing pushdown in the extractor — so the cross-source merge can
	// align and deduplicate this stream against its store counterpart and replicas.
	hash   uint64
