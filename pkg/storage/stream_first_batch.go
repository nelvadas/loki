...
func newStreamFirstSampleBatchIterator(
	...
	for fp, seriesChunks := range byFingerprint {
		...
		// Identify each stream by its fingerprint, the raw per-stream identity available from the
		// index ref (the chunk's Metric labels are not populated until the chunk is loaded). This
		// is what the cross-source merge orders and deduplicates by, so overlapping copies of a
		// stream align; the extractor's reduced labels are used only by the bucketing evaluator.
		groups = append(groups, streamFirstChunks{hash: uint64(fp), chunks: flat})
	}

	// Order streams by the streamHash the iterator exposes so the querier's stream-first merge can
	// align this tier's streams with the others.
